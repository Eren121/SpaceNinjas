...
{
    m_wrapped = SDL_JoystickOpen(device_index);
    
    if(!m_wrapped)
    {
        // Check the joystick was successfully opened
        std::cerr << "Error while opening the joystick: " << SDL_GetError() << std::endl;
    }
}

SDL::Joystick::~Joystick()
{
    if(SDL_JoystickGetAttached(m_wrapped))
    {
        SDL_JoystickClose(m_wrapped);
        m_wrapped = nullptr;
    }
}

SDL::Joystick::operator SDL_Joystick *() const
{
    return m_wrapped;
}

SDL::Joystick::operator bool() const
{
    return m_wrapped;
}

SDL::GameController::GameController(int device_index)
{
    m_wrapped = SDL_GameControllerOpen(device_index);
    
    if(!m_wrapped)
    {
        // Do not throw but check for error if not successfully opened
        std::cerr << "Error while opening the controller: " << SDL_GetError() << std::endl;
    }
}

SDL::GameController::~GameController()
{
    if(SDL_GameControllerGetAttached(m_wrapped))
    {
        SDL_GameControllerClose(m_wrapped);
        m_wrapped = nullptr;
    }
}

SDL::GameController::operator SDL_GameController *() const
{
    return m_wrapped;
}

SDL::GameController::operator bool() const
{
    return m_wrapped;
}
