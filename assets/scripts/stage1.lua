...
function phase_rain(arg)
    ...
    while api.time < end_time do
        ...
        body.velocity = vec2.new(0, -velocity)
...
function phase_follow(arg)
    ...
    while api.time < end_time do
        ...
        local pos = get_circle_pos(random_angle())
...
function stage1()
    ...
    phase_rain{duration=Time.seconds(30), velocity=10}
    phase_follow{duration=Time.seconds(20), period=Time.milliseconds(500)}
    wait(Time.seconds(3))
