...
/// @brief Lazy resource
/// @details Sometimes you want a class to compute some actions only when update, after the user has set all
/// the relevant parameters (like many Drawable classes). C++ "mutable" keyword can be used for lazy resources, and
/// this class is a generic implementation of a lazy resource.
/// @remarks On instanciation, the resource is marked as needing an update.
class LazyResource
{
protected:
